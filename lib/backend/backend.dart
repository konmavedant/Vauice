...
import 'schema/selected_talent_record.dart';
import 'dart:async';
...
export 'schema/selected_talent_record.dart';
...
Future<FFFirestorePage<ProfileInfoRecord>> queryProfileInfoRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, ProfileInfoRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      ProfileInfoRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<ProfileInfoRecord> data) {
          for (var item in data) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
...
Future<FFFirestorePage<TalentsRecord>> queryTalentsRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, TalentsRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      TalentsRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<TalentsRecord> data) {
          for (var item in data) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
...
/// Functions to query SelectedTalentRecords (as a Stream and as a Future).
Future<int> querySelectedTalentRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      SelectedTalentRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<SelectedTalentRecord>> querySelectedTalentRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      SelectedTalentRecord.collection,
      SelectedTalentRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<SelectedTalentRecord>> querySelectedTalentRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<SelectedTalentRecord>> querySelectedTalentRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, SelectedTalentRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      SelectedTalentRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<SelectedTalentRecord> data) {
          for (var item in data) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
